...
build: restore
	dotnet build ./jaytwo.StreamFile.sln
...
unit-test: build
	...
	rm -rf out/coverage
	cd ./test/jaytwo.StreamFile.Tests; \
		dotnet test \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.StreamFile.Tests.trx"
	reportgenerator \
